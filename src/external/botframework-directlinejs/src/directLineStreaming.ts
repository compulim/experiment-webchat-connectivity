...
export class DirectLineStreaming implements IBotConnection {
  ...
  public reconnect({ conversationId, token }: Conversation) {
    // ***
    console.log('Streaming: reconnect() called.', this.connectionStatus$.getValue(), this.connectDeferred);
    ...
    // ***
    console.log('Streaming: end() called.');
  ...
  private async connectAsync() {
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        // ***
        console.log('Streaming: Connecting.');
        await this.streamConnection.connect();

        // ***
        // console.log('Streaming: Connected, pausing.');

        // *** Added a sleep before sending first Web Socket message.
        // await new Promise(resolve => setTimeout(resolve, 3000));

        // ***
        console.log('Streaming: Connected, handshaking.');
        ...
        // ***
        console.log('Streaming: Got handshake response.', response.statusCode, response.streams.length);
        ...
        // ***
        console.log('Streaming: Connected.', conversation);
        ...
        // ***
        console.log('Streaming: Errored out.', e);
        reject(e);
      }
    });
  }
  ...
  private async connectWithRetryAsync() {
    ...
    for (;;) {
      this.connectDeferred = createDeferred();

      // ***
      console.log('DLJS: Wait for kickoff.', this.connectDeferred);
      ...
      while (numRetries > 0) {
        ...
        // ***
        console.log('DLJS: Connection interrupted.', { connectionStatus: this.connectionStatus$.getValue() });
        ...
        if (60000 < Date.now() - start) {
          ...
          // ***
          console.log('DLJS: Not stable, sleeping.', delay);
